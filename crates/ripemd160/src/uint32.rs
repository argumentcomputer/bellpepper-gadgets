...
impl UInt32 {
    ...
    pub fn shl(&self, by: usize) -> Self {
        ...
        let new_bits: Vec<_> = std::iter::repeat(&fill)
            .take(by)
            .chain(self.bits.iter()) // The bits are least significant first
            .take(32) // Only 32 bits needed!
            .cloned()
            .collect();
        UInt32 {
        ...
        CS: ConstraintSystem<Scalar>,
    {
        Self::triop(
            ...
            c,
            |a, b, c| (a & b) | (!b & c),
            |cs, i, a, b, c| Boolean::ripemd160_d1(cs.namespace(|| format!("d1 {}", i)), a, b, c),
        )
    }
        ...
        CS: ConstraintSystem<Scalar>,
    {
        Self::triop(
            ...
            c,
            |a, b, c| (a) ^ (b | !c),
            |cs, i, a, b, c| Boolean::ripemd160_d2(cs.namespace(|| format!("d1 {}", i)), a, b, c),
        )
    }
...
mod test {
    ...
    fn test_uint32_shl() {
        ...
        for _ in 0..50 {
            for i in 0..60 {
                let num = rng.next_u32();
                let a = UInt32::constant(num).shl(i);
                let b = UInt32::constant(num.wrapping_shl(i as u32));

                assert_eq!(a.value.unwrap(), num.wrapping_shl(i as u32));
    ...
    fn test_uint32_ripemd_d1() {
        ...
        for _ in 0..1000 {
            ...
            let r = UInt32::ripemd_d1(&mut cs, &a_bit, &b_bit, &c_bit).unwrap();
    ...
    fn test_uint32_ripemd_d2() {
        ...
        for _ in 0..1000 {
            ...
            let r = UInt32::ripemd_d2(&mut cs, &a_bit, &b_bit, &c_bit).unwrap();
