...
impl<F: PrimeField + PrimeFieldBits> G2Point<F> {
        ...
        CS: ConstraintSystem<F>,
    {
        ...
        // FIXME: the below isn't working: .neg() doesn't work for constant elements some weird reason? this might be an issue with bellpepper-emulated?
        // let xi = Fp2Element::from_dec(("2", "1")).unwrap();
        // d("xi", &xi);
        // let xi = xi.neg(&mut cs.namespace(|| "xi <- (-2, -1)"))?;
        // d("xi", &xi);

        let xi = Fp2Element::from_dec(("4002409555221667393417789825735904156556882819939007885332058136124031650490837864442687629129015664037894272559785", "4002409555221667393417789825735904156556882819939007885332058136124031650490837864442687629129015664037894272559786")).unwrap();
